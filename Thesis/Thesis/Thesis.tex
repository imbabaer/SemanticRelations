...
In diesem Kapitel sollen die unterschiedlichen Word2Vec Modelle mit ihren Trainingskorpora (Gesamtkorpus\footnote{Siehe \ref{sec:Gesamtkorpus} Gesamtkorpus} und Techkorpus\footnote{Siehe \ref{sec:Teilkorpus} Teilkorpus}) untersucht werden. Dies soll durch fünf Experimente realisiert werden. In allen Experimenten werden semantischen Beziehungen zwischen Wörtern untersucht. Jedes Experiment ist in drei Teile aufgeteilt, Beschreibung, Durchführung und Interpretation/Ergebnis.\\
...
\end{table}
 ...
 In der Tabelle \ref{tab:bspsynoyme} sind alle gefundenen Synonyme und die dazugehörigen Testwörter, sowie der Korpus in dem sie auftreten, aufgelistet.
		...
		Wie aus den Daten hervorgeht, werden zwar einige Synonyme der Suchbegriffe gefunden, allerdings jedoch nicht viele. Ein Grund, der die geringe Anzahl an Synonymen erklären könnte, könnte mit der räumlichen Darstellung der Worte im Word2Vec Modell zusammenhängen. Die ähnlichen Worte ($W_1$) eines Suchbegriffs ($O$) haben einen minimalen räumlichen Abstand zum Suchbegriff (O). 
		Die durch die Rekursion erhaltenen ähnlichen Worte ($W_2$) der ähnlichen Worte ($W_1$), haben wieder einen minimalen räumlichen Abstand (zu $W_1$), allerdings ist der Abstand vom ursprünglichen Suchbegriff ($O$) zu den in $W_2$ enthaltenen Wörter größer als zu den Worten aus $W_1$, denn sonst wären sie schon in der Menge $W_1$ enthalten. Also sind diese Wörter noch weiter vom ursprünglichen Suchbegriffs ($O$) entfernt und somit unähnlicher.\\
	...
	\section{Konkretisierungen}
		\subsection*{Beschreibung}
		In diesem Experiment soll das domänenspezifischen Technologiemodell untersucht werden. Es wird untersucht, ob die ähnlichen Wörter der Testdaten eine Konkretisierung des jeweiligen Suchbegriffs darstellen.\\
		Dies ist gerade bei mehrdeutigen Wörtern interessant, da hier dann eine Sinnrichtung in den ähnlichen Wörtern dominant vertreten sein könnte und somit klar ist, in welchem Kontext das mehrdeutige Wort in dieser Domäne gemeint ist.\\
		\subsection*{Durchführung}
